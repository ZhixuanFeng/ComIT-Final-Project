...
function setPlayerBlock(block) {
    this._blockText.setText(block);
}

function updatePlayerBlock() {
    let availableBlock = 0;
    hand.forEach(function (card) {
        if (card.info.block) availableBlock += card.info.block;
    });
    this.setBlock(availableBlock);
}
...
function Player(aGame, aParent, aName, aAddToStage, aEnableBody, aPhysicsBodyType, x, y, playerInfo) {
    ...
    this._hpText = this.game.add.text(20.0, 0.0, playerInfo.hp + '/' + playerInfo.maxHp, {"font":"bold 20px Arial"}, _stats);
    this._hpText.scale.setTo(0.75, 0.75);

    this.game.add.sprite(0.0, 15.0, 'card', 'mana', _stats);

    this._manaText = this.game.add.text(20.0, 15.0, playerInfo.mana + '/' + playerInfo.maxMana, {"font":"bold 20px Arial"}, _stats);
    this._manaText.scale.setTo(0.75, 0.75);

    this.game.add.sprite(0.0, 30.0, 'card', 'hate', _stats);

    this._hateText = this.game.add.text(20.0, 30.0, '0', {"font":"bold 20px Arial"}, _stats);
    this._hateText.scale.setTo(0.75, 0.75);

    this.game.add.sprite(0.0, 46.0, 'card', 'block', _stats);

    this._blockText = this.game.add.text(20.0, 45.0, '0', {"font":"bold 20px Arial"}, _stats);
    this._blockText.scale.setTo(0.75, 0.75);
...
Player.prototype.setBlock = setPlayerBlock;
Player.prototype.updateBlock = updatePlayerBlock;
